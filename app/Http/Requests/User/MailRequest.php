...
class MailRequest extends FormRequest
{
    ...
    {
        if ($this->has('id')) {
            $rules['email'] = ['required', 'email', 'max:32', "unique:users,email,{$this->input('id')},id,deleted_at,NULL"];
        } else {
            $rules['email'] = ['required', 'email', 'max:32', 'unique:users,email,NULL,id,deleted_at,NULL'];
        }  
