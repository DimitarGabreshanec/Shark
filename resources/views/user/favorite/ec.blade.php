...
@section('content')
    <main>
            <nav>
                <ul class="nav mb20">
                    <li><a href="{{ route('user.favorite.index', ['product_type' => config('const.product_type_code.fix')]) }}">お店</a></li>
            ...
            <ul class="shop-list-search">
                @php
                    ...
                    $stores = \App\Service\StoreService::getFavoriteStores(config('const.product_type_code.ec'));
                @endphp
                @foreach($stores as $store)
                    ...
                    @endphp
                    <li>
                        <a href="{{ route('user.stores.store_products', ['store' => $store, 'product_type'=> config('const.product_type_code.ec')]) }}">
                            <div class="pic"><span>{{number_format($store->getProductCountByType(config('const.product_type_code.ec'))) . '商品'}}</span>
                            <i class="favorite active" onclick="location.href='{{ route('user.favorite.index', ['product_type'=> config('const.product_type_code.ec')]) }}'; toggleFavorite('{{ $store->id }}', '{{config("const.product_type_code.ec")}}')">
                            </i>
